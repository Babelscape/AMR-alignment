...
from spring_amr.evaluation import write_predictions, compute_smatch, predict_amrs, predict_sentences, compute_bleu, predict_sentences
from spring_amr.utils import instantiate_model_and_tokenizer, instantiate_loader, instantiate_aligner_loader
...
def do_train(checkpoint=None, direction='amr', split_both_decoder=False, fp16=False):
    ...
    train_loader = instantiate_aligner_loader(
        config['train'],
        tokenizer,
    ...
    dev_loader = instantiate_aligner_loader(
        config['dev'],
        tokenizer,
        batch_size=config['batch_size'],
    ...
    if direction == 'amr':
        ...
        def train_step(engine, batch):
            ...
            loss = output["loss"] + 0.1 * output['loss_align']
    ...
    if not config['best_loss']:
        if direction in ('amr', 'both'):
            ...
            def smatch_eval(engine):
                ...
                if tokenizer.tgt_lang == "en_XX":
                    decoder_token = 0
                else:
                    decoder_token = tokenizer.convert_tokens_to_ids("en_XX")

                graphs = predict_amrs(dev_loader, model, tokenizer, decoder_start_token_id=decoder_token, restore_name_ops=config['collapse_name_ops'])
        ...
        if direction in ('text', 'both'):
            ...
            def smatch_eval(engine):
                device = next(model.parameters()).device
                dev_loader.device = device
                if tokenizer.src_lang == "en_XX":
                    decoder_token = 0
                else:
                    decoder_token = tokenizer.convert_tokens_to_ids(tokenizer.src_lang)

                pred_sentences = predict_sentences(dev_loader, model.rev, tokenizer, decoder_start_token_id=decoder_token, beam_size=config['beam_size'])
