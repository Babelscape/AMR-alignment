...
import regex as re
import torch
from transformers.models.mbart.tokenization_mbart import *
...
class AMRMBartTokenizer(MBartTokenizer):
    ...
    def __init__(self, *args, use_pointer_tokens=False, collapse_name_ops=False, direction="amr" , **kwargs):
    ...
    def pre_tokenize(self, text):
        ...
        for tok_span in text.lstrip().split(' '):
            ...
            if len(recats) == 2 and recats[0] in self.recategorizations and (self.convert_tokens_to_ids('_' + recats[1]) != self.unk_token_id):
        ...
        for i, (backr, tokk) in enumerate(zip(backreferences, linearized_nodes)):
            ...
            if tokk.startswith('"') and tokk.endswith('"'):
                ...
                if is_in_enc:
                    ...
                    rel = tokk[:-3]
                    if self.unk_token != self.INIT + rel and self.convert_tokens_to_ids(self.INIT + rel) != self.unk_token_id:
                        bpe_toks = [self.INIT + rel, '-of']
                    else:
                        bpe_toks = [self.INIT + ':'] +  [tokkk[1:] for tokkk in self._tok_bpe(tokk[1:])] + ['-of']
                elif is_rel:
                    bpe_toks = [self.INIT + ':'] + [tokkk[1:] for tokkk in self._tok_bpe(tokk[1:])]
                else:
                    raise
    ...
    def batch_encode_sentences(self, sentences, device=torch.device('cpu')):
        ...
        if self.direction == "text":
            batch = super().batch_encode_plus(sentences, return_tensors='pt', padding=True)
...
class BMRMBartTokenizer(AMRMBartTokenizer):
    def init_amr_vocabulary(self, pred_min=0):
        self.vocab_path = "data/vocab/bmr/"
        super().init_amr_vocabulary(pred_min=pred_min)
        new_special_tokens = []
        for line in Path(ROOT/f'{self.vocab_path}special_tokens.txt').read_text().strip().splitlines():
            ...
            if tokk.startswith('"') and tokk.endswith('"'):
                ...
                if is_in_enc:
                    ...
                    if self.unk_token != self.INIT + rel and self.convert_tokens_to_ids(self.INIT + rel) != self.unk_token_id:
                        ...
                        bpe_toks = [self.INIT + ':'] +  [tokkk[1:] for tokkk in self._tok_bpe(tokk[1:])] + ['_of']
                elif is_rel:
                    bpe_toks = [self.INIT + ':'] + [tokkk[1:] for tokkk in self._tok_bpe(tokk[1:])]
                elif is_bn:
